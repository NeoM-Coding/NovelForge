...
- Production server: `@hono/node-server` (via `dist/api/server.js`, launched with `--experimental-loader ./loader.mjs`)
...
### DevOps
- Docker + Docker Compose (compose files in `docker/`)
- Nginx reverse proxy + Let's Encrypt SSL
- Deployment: Alibaba Cloud ECS (single instance, directory `/www/wwwroot/NovelForge`)
...
│   ├── routers/            # novel, chapter, translate, generate, lore, rag, tag, material, annotation, upload, trope
...
├── docker/                 # Docker Compose files + Dockerfile + nginx.conf
│   ├── docker-compose.yml           # Production (build from source)
│   ├── docker-compose.image.yml     # Pre-built image from GHCR
│   ├── docker-compose.local.yml     # Local pre-built image import
│   ├── Dockerfile
│   └── nginx.conf
├── loader.mjs              # ESM import path resolver for production Node runtime
├── vite.config.ts
├── drizzle.config.ts
...
| `npx vitest run src/components/MyComponent.test.ts` | Run a single test file |
...
| `./start.sh` | One-click launch with pre-built image (Linux/macOS/WSL) |
| `./start.sh build` | One-click launch with local build |
| `docker compose -f docker/docker-compose.image.yml up -d` | Start with pre-built image |
| `docker compose -f docker/docker-compose.yml up -d --build` | Start with local build |
...
This emits `dist/api/server.js` and other compiled server files. The `Dockerfile` runs both `npm run build` (frontend) and the backend compilation. Production runtime uses `node --experimental-loader ./loader.mjs dist/api/server.js` to resolve path aliases.
...
- `dev` is the active development branch. Daily work merges here first.
...
**Workflow**: `feat/xxx` → merge to `dev` → local test → merge to `master` → ECS deploy
